...
public class ResponseParser {
    ...
    public static List<Finding> parseJsonResponse(String rawJson) {
        ...
        try {
            return parseCleanJson(cleaned);
        } catch (com.google.gson.JsonSyntaxException | ParseException firstAttempt) {
            // The model likely leaked reasoning/prose ahead of (or around) the real JSON
            // answer - e.g. "The user wants me to review... {\"findings\": [...]}" - even
            // with reasoning disabled, some models still narrate a little. As a fallback,
            // find the outermost {...} or [...] substring and try parsing just that.
            String extracted = extractEmbeddedJson(cleaned);
            if (extracted != null) {
                try {
                    return parseCleanJson(extracted);
                } catch (com.google.gson.JsonSyntaxException | ParseException secondAttempt) {
    ...
    private static List<Finding> parseCleanJson(String cleaned) {
    ...
    /** Finds the first '{' or '[' and its matching close bracket, to pull JSON out of surrounding prose. */
    private static String extractEmbeddedJson(String text) {
        int objStart = text.indexOf('{');
        int arrStart = text.indexOf('[');
        int start = (objStart == -1) ? arrStart : (arrStart == -1 ? objStart : Math.min(objStart, arrStart));
